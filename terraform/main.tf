...
resource "aws_ecs_cluster" "coffee" {
  name = "coffee"
}

resource "aws_ecr_repository" "coffee" {
  name                 = "coffee"
...
module "webserver" {
  ...
  ecr_repository = aws_ecr_repository.coffee.repository_url
  ecs_cluster    = aws_ecs_cluster.coffee.id
